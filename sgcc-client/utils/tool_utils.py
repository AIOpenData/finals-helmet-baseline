...
def set_random_seed(args):
    ...
    # ensure each convolution operation of CuDNN is consistent
    torch.backends.cudnn.deterministic = True

    # release graphics cache
    torch.cuda.empty_cache()


@contextmanager
def timer(name, logger, func=None):
    """ a customed timer function """
    t1 = time.time()
...
def load_json(json_file_name):
    return CommonUtils.get_object_by_json_file_func(read_file_path=json_file_name)
...
def xywh2xyxy(x):
    """ transfer from (center x, center y, width, height) to (x1, y1, x2, y2) """
    y = x.new(x.shape)
    y[..., 0] = x[..., 0] - x[..., 2] / 2
    y[..., 1] = x[..., 1] - x[..., 3] / 2
    y[..., 2] = x[..., 0] + x[..., 2] / 2
...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    # create recall-precision and compute AP for each class
    ap, p, r = [], [], []
...
def non_max_suppression(prediction, conf_threshold=0.5, nms_threshold=0.4):
    """ removes detections with lower object confidence score than 'conf_threshold'
    ...
    for image_i, image_pred in enumerate(prediction):
        ...
        while detections.size(0):
            large_overlap = bbox_iou(detections[0, :4].unsqueeze(0), detections[:, :4]) > nms_threshold
